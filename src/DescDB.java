...
public class DescDB extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		//Creating session and retreiving sesion
		...
		//Session Exists
			...
			//Establishing Connection
				...
				error_occured = true;
                                System.out.println("Exception in DescDB catch");
				error_message = e.toString();
			}
			/*On Successful Connection
                        *       Retreive Table
                        *               and Column informations */
			if(!error_occured) {
				try {
					...
					//Iterating resultset and retreiving table information
						...
						//Updating resultset with column information
                                                if(schemaValue == 0)	
                                                	rs = dbMetaData.getColumns(schema,null,tname,null);
                                                else					
                                                	rs = dbMetaData.getColumns(null,schema,tname,null);
                                                
                                                //Iterating resultset and retreiving number of columns
					...
					error_message = e.toString();
                                ...
                                //Closing resultset and connections
				try {
					resultSet.close();
					connection.close();
				}	
			...
			if(error_occured) {
				...
				//Displaying Table Names and Corresponding operations 
                                for(int i=0;i<tableNames.size();i++) {
					String tname = tableNames.elementAt(i).toString();
					writer.println("<TR>");
					writer.println(	"<TD id=common_td><A target=right  HREF='DescTab?table_name=" + tname + 
									"'>" + tname + "</A></TD>");

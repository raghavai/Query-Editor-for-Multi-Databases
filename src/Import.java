	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		HttpSession session	  = req.getSession(false);
		  ...
		  //Session Timed Out
			...
			//Uploading file
                        try {
				//Creating an object of DiskFileUpload class
                                upload = new DiskFileUpload();			
                                
                                //Retreiving the list of files to be uploaded
                                ...
                                while (iter.hasNext()) {
                                    ...
                                    //Creating file item for each list item
                                    FileItem item = (FileItem) iter.next();
					// If the field is an upload field and not a form field
                                        if(!item.isFormField())	{
			...
			if(!error_occured)	{
				try {
					...
					error_message = e.toString();
                        ...
                        //Redirecting to Import page along with error message if error occured
			if(error_occured) 
                            res.sendRedirect("Import?message=" + error_message);
			else {
				try {
                                        //Execute imported query
					while(true) {
					...
					//Enabling error_occured if imported query fails
                                        error_occured = true;
					error_message = e.toString();
				}
			}
			...
			//Imported Query failed
                        if(error_occured) {
                                //List the Database if table is created
				if(tableCreated) {
					res.setContentType("text/html");
					PrintWriter writer = res.getWriter();
					...
					writer.close();
                        ...
                        //Imported Query Successful
			else {
				//List Database tables if table is created
                                if(tableCreated) {
					res.setContentType("text/html");
					PrintWriter writer = res.getWriter();
				...
				//Redirect to import along with success message
                                else
                                    res.sendRedirect("Import?message=Import succeeded");
			}
		}
	}
}
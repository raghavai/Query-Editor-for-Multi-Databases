...
public class DescTab extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		if(session == null) {
			...
			System.out.println("ssss\t"+tableName);
			...
			if(!error_occured) {
				...
				if(!error_occured) {
					//Retreiving Primary Key columns 
                                        try {
						...
						if(resultSet != null) resultSet.close();
                                        ...
                                        //Retreiving Foreign Key Columns
					try {
						if(schemaValue == 0) {
							resultSet = dbMetaData.getCrossReference(schema,null,tableName,schema,null,tableName);
						}
						else {
							resultSet = dbMetaData.getCrossReference(null,schema,tableName,null,schema,tableName);
						}
			...
			writer.println(	"<TR><TD>&nbsp&nbsp&nbsp&nbsp" +
							...
							"<A target=top HREF='TabQuery?table_name=" + tableName + "'>" + 
							"<img onMouseOver='putOn(this,4)' onMouseOut='putOff(this,4)' " +
							"name=pic4 src='pics/sql1.jpg' border=0 " +
							"width=80 height=26 align=absbottom></A>" +
							"<A target=top HREF='InsertForm?table_name=" + tableName + "'>" + 
			...
			if(error_occured) {
				...
				if(message != null)	{
					...
					writer.println("<HR WIDTH=100%><BR>");
                                ...
                                //Displaying Columns and Column operations

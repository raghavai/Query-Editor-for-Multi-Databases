		...
		if(session == null) {
			...
			//Connection Successful
				...
				try {
					//Updating ResultSet with Column Information
                                        if(schemaValue == 0) resultSet = 
                                                dbMetaData.getColumns(schema,null,tableName,null);
					else			
                                                resultSet = dbMetaData.getColumns(null,schema,tableName,null);
                                        
                                         //Iterating Result Set and obtaining Column Names,types,sizes
					while(resultSet.next()) {
						colNames.add(resultSet.getString(4));
						int type = resultSet.getInt(5);
                                                
                                                //Enabling lob exists if type of column is any of the binary types,blob or clob type
						if(!lobExists)	{
							switch(type) {
			...
			if(error_occured) {
				...
				writer.println("</TABLE><BR>");
                                ...
                                //Submit insert form to InsertLob if Lob Exists
				if(lobExists) {
					...
					writer.println("</TR>");
					//Displaying column names,types,sizes
					...
					writer.println("</FORM>");
                                ...
                                //Submitting InsertForm to InsertNorm if no lobs
				else {

...
public class EditNorm extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if(session == null) {
			...
			if(!error_occured) {
				try {
					//Updating resultset with column information
                                        if(schemaValue == 0) 
			...
			if(!error_occured)	{
				...
				for(int i=0;i<rowCount;i++) {
					//Retreiving Previous row data 
                                        prevData[i] = req.getParameterValues("row_" + i + "_prev_data");
					//Retreiving Current row data 
					...
					for(int x=0;x<columnCount;x++) {
						...
						//status of current value at 'i'th row and 'x'th column
                                                boolean isOk = true;
						...
						dataStatus[i][x] = isOk;
                                        ...
                                        //Adding valid rows to valid Index
					if(thisRowOk)	
                                            validIndex.add(new Integer(i));
					//Adding Invalid rows to invalid index
                                        else
                                            invalidIndex.add(new Integer(i));
				}
				...
				for(int x=0;x<validIndex.size();x++)	{
					...
					for(int j=0;j<columnCount;j++)	{
						...
						int type = Integer.parseInt(colTypes.elementAt(j).toString());
                                                ...
                                                //Updating set clause and where clause
                                                switch(type) {
							case Types.BIT		:	
							case Types.SMALLINT :
							case Types.TINYINT	:	
					...
					//Executing query
						...
						//Incrementing updated rows
                                                updatedCount ++;
					}
					catch(SQLException se)	{	
                                            //On Failure adding error to errorindex
			...
			if(error_occured)
                            writer.println("<H3>" + error_message + "</H3></BODY></HTML>");
                                ...
                                //Displaying Number of Updated rows
                                if(updatedCount > 0) {

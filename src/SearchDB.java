...
public class SearchDB extends HttpServlet {
		...
		if(session == null) {
			...
			try {
				...
				if(schemaValue == 0)	resultSet = dbMetaData.getTables(schema,null,null,types);
                                ...
                                //Iterating resultset and retreiving table information
				while(resultSet.next()) tableNames.add(resultSet.getString(3));
				if(resultSet != null)	resultSet.close();
			}
			catch(Exception e) {
				//Enabling error_occured when conection failed
                                error_occured = true;
			...
			if(!error_occured) {
				...
				//Where to look for (Columns,data,both)
                                writer.println(	"<TR><TH id=common_hed STYLE='font-weight:bold;text-align:center'>" +
								" LOOK FOR </TH></TR>");
				writer.println(	"<TR><TD id=common_td STYLE='font-weight:bold;text-indent:100px'>" +
								"<INPUT TYPE=radio NAME=search VALUE=0 CHECKED> Column Names Only </TD></TR>");
				writer.println(	"<TR><TD id=common_td STYLE='font-weight:bold;text-indent:100px'>" +
								"<INPUT TYPE=radio NAME=search VALUE=1> Data Only </TD></TR>");
				writer.println(	"<TR><TD id=common_td STYLE='font-weight:bold;text-indent:100px'>" +
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		PrintWriter writer  = res.getWriter();
                ...
                //Redirecting to Login when session failed
		if(session == null) {
			writer.println("<HTML>");
			writer.println(	"<BODY onLoad=\"window.parent.location.href='Login?message=" +
							"Sorry Your session expired. Please Login Again.'\"");
			...
			try {
				...
				//Updating resultset with table information
                                ...
                                //Iterating resultset to retreive table names
				while(resultSet.next()) tableNames.add(resultSet.getString(3));
				if(resultSet != null)	resultSet.close();

				//Search for data
				if(search != 0) {
					//Search key (case sensitive or not)
                                        String newKey = (matchCase) ? keyword : keyword.toUpperCase();
                                        //Searching selected tables
					for(int i=0;i<selectedTables.length;i++) {
						...
						try	{	
							...
							//Itertaing Columns of selected tables
							while(resultSet.next()) {
								for(int x=0;x<columnCount;x++) {
									...
									//Updating matchedData if search key found
                                                                        if(data.startsWith(newKey)) {
					...
					//Iterating resulset and rereiving column infomation
                                        while(resultSet.next()) {
						String tabName = resultSet.getString(3);
						String colName = resultSet.getString(4);
                                                //Searching columns of selected table
						for(int i=0;i<selectedTables.length;i++) {
                                                    //Updating matched columns if any column matches keyword
			...
			if(matchCount > 0) {
				...
				writer.println("<TABLE ALIGN=center CELLSPACING=1 CELLPADDING=4 BORDER=0 WIDTH=100%");
				writer.println("<TR>");
				//Displaying match count for keyword

...
public class EditLobs extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if(session == null) {
			...
			if(!error_occured)	{
				int index = 0;
				//Iteratior for list of items
                                Iterator iter = items.iterator();
                        
                        //Iterating Item List
		        while (iter.hasNext()) {
			        
                                        //Retreiving FileItem from Items list
					...
					//Retreiving file name
						...
						if(fname.equals("row_ids"))		
                                                    rowids = value;
						else if(fname.equals("sort_column"))
                                                    sortColumn = value;
						else if(fname.equals("sort_order"))	
                                                    sortOrder = Integer.parseInt(value);
						else if(fname.equals("curr_id"))	
                                                    currId = Integer.parseInt(value);
				...
				//Retreiving Lob Data to edit from table
				if(rowOk) {
					try {
					...
					Vector lobIndex = new Vector();
                                        //Iterating Columns and updating set clause
					for(int i=0;i<columnCount;i++)	{
			...
			if(error_occured) {
				...
				writer.println("</TABLE><BR>");				
                                ...
                                //Displaying updation success for one row
				if(updatedCount == 1) {
					...
					writer.println("<HR WIDTH=100%>");
                                ...
                                //Displaying error message if updation caused errors
					...
					writer.println("<HR WIDTH=100%>");
                                ...
                                //Displaying error message if updated data is invalid

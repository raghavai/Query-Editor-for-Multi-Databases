		...
		PrintWriter writer = res.getWriter();
		 ...
		 //Session Timed Out
			...
			try {
				...
				//Retreiving metadata for connection
                                dbMetaData	= connection.getMetaData();
			}
			catch(Exception e) {
				//Enabling error_occured on COnnection Failure
                                error_occured = true;
			...
			if(!error_occured) {
				try {
                                        ...
                                        //Ipdating resultset with column information
                                        if(schemaValue == 0)
					...
					//Enabling error occurred if any error occurs during ResultSet Rereival
                                        error_occured = true;
					error_message = e.toString();
				}
			}
			...
			if(!error_occured)	{
				...
				//Retreiving current row values
                                for(int i=0;i<rowCount;i++) {
					String[] thisRowData = req.getParameterValues("row" + i + "data");
                                        
                                        //Column status of current row
					...
					for(int x=0;x<columnCount;x++) {
						...
						int nullable= Integer.parseInt(Nullable.elementAt(x).toString());
                                                ...
                                                //Enable isOK if value is case insensitive and not nullable
						if(value.equalsIgnoreCase("null") && nullable != 0) isOk=true;
							...
							//Validate column type
								...
								case Types.VARCHAR	:	isOk = true;						break;
                                                 ...
                                                 //Verifying null values and also whether the column is not nullable
						if(value.equals("") && nullable!= 0)	isOk = true;
						
                                                //Disabling row if data is not validated
                                                if(!isOk)	thisRowOk = false;
						
                                                thisRowStatus[x] = isOk;
					}
                                        
                                         //Adding valid rows to valid row data
					if(thisRowOk) 
                                                validRowData[validCount++] = thisRowData;
					//Adding Invalid rows to invalid row data and increasing the number of invalid row status
                                        else {
				...
				for(int i=0;i<validCount;i++)	{
					...
					//Updating insert statement 
						...
						//Execute Insert statement and increment the count of inserted rows
                                                try {
							int count = statement.executeUpdate(insertQuery);
							insertedCount += count;
						}
                                                //Update error Index if insertion failed
							...
							errorIndex.add(new Integer(i));
                                ...
                                //Closing resulstset and connection
				try {
			...
			//Display error message if error occured
				...
				writer.println("<HTML><HEAD><H3>" + error_message + "</H3></HEAD></HTML>");
                                ...
                                //Displaying prerequisites of page
                                writer.println("<HTML>");
				writer.println("<HEAD>");
				...
				writer.println("</TABLE><BR>");				
                                ...
                                //Displaying number of rows inserted
				if(insertedCount > 0) {
					...
					writer.println("<HR WIDTH=100%>");
                                ...
                                //Displaying  the rows that caused error
				if(invalidCount > 0 || errorIndex.size()>0)	{
					...
					if(invalidCount > 0) {
						...
						for(int i=0;i<invalidCount;i++) {
							...
							writer.println("</TR>");
                                         ...
                                         //Rows caused error during updation
					...
					writer.println("</FORM>");
                                ...
                                //On successful insertion display the insert form
                                else {
					writer.println("<FORM NAME=insert_form METHOD=post ACTION='InsertNorm'>");
					writer.println("<INPUT TYPE=hidden NAME=table_name VALUE=" + tableName + ">");

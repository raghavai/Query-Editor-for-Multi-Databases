...
public class LoadAll extends HttpServlet
{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			//Enabling error occured if connection failed,update error message with corresponding error
                        error_occured = true;
			error_message = e.toString();
			wri.println("Error Occured");
		}
                
                //Connection Successful
		if(!error_occured) {
			try {
				//Retrieving ProductName and Product Version
				...
				//Setting value of schema
                                if(genericTerms.indexOf(","+schemaTerm.toUpperCase()+",") != -1)
                                	schemaValue = 1;
				else {
				...
				String pname = dbProductName.toUpperCase();
                                ...
                                //Updating Schema accordingly with the database selected
                                if(pname.indexOf("ORACLE") != -1)	
                                        schema = userid.trim().toUpperCase();
					...
					if(resultSet.next()) {
						...
						schema = schema.substring(schema.lastIndexOf("\\")+1,schema.length());	
                                ...
                                System.out.println("selected data base schema  =="+schema);
                                //Creating session object and retreiving session if any or creating a new session
			...
			try {
                ...
                //Displaying error message and redirecting to login page if any error occurs
		if(error_occured) {
			writer.println("<HTML>");
			...
			writer.println("</HTML>");
                ...
                //On successful login display database information and content

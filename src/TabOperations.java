...
public class TabOperations extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		if(session == null) {
			...
			//Assigning selected operation accordingly
			...
			try {
				...
				dbMetaData = connection.getMetaData();
                                //Updating resultset with data types available in database
				resultSet  = dbMetaData.getTypeInfo();
                                
                                //Iterating resultset to retreive type names
				...
				if(schemaValue == 0)	resultSet = dbMetaData.getColumns(schema,null,tableName,null);
                                ...
                                //Iterating resultset to retreive column information
				...
				if(resultSet != null) resultSet.close();
                                ...
                                //Primary Key retreival 
				try {
                                        //Updating resultset with primary key information
                                        ...
                                        //Iterating resultset to retreive primarykey columns
			...
			if(!error_occured) {
				...
				writer.println(	"<SELECT NAME=column_name STYLE='background:azure;width:180px' " +
                                ...
                                //Selecting Column to be altered
                                writer.println("<OPTION VALUE=0> --------- Select a Column --------- </OPTION>");
				for(int i=0;i<colNames.size();i++) {
					String name = colNames.elementAt(i).toString();
					writer.print("<OPTION VALUE=\"" + name + "\" "); 
					//Retreiving column type,size and nullable information of selected column
				...
				//Displaying type name of column
				...
				//Checkbox for primary key information
				...
				writer.println("<TD id=common_data STYLE='text-align:center;font-weight:bold'>");
				//Checkbox for unique constraint information
                                writer.println("<INPUT TYPE=checkbox NAME=unique VALUE=1>  Unique </TD>");

				writer.println("<TD id=common_data STYLE='text-align:center;font-weight:bold'>");
				//Checkbox for nullable information of column
				...
				//Alter Column image when clicked alters column
				...
				//Selecting Column to be renamed
                                writer.println("<OPTION VALUE=0> --------- Select a Column --------- </OPTION>");
				for(int i=0;i<colNames.size();i++) {
					String name = colNames.elementAt(i).toString();
					writer.print("<OPTION VALUE=\"" + name + "\" "); 
				...
				//Rename Column image when clicked renames column
				...
				writer.println("</SELECT></TD>");
				//Drop image when clicked drops the column
